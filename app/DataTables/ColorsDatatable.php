	...
	public function dataTable($query) {
		return datatables($query)
			->addColumn('checkbox', function($color){
				return '<input type="checkbox" name="item[]" value="'.$color->id.'" class="item_checkbox"/>';
			})	
			->addColumn('edit', function($color){
				return '<a href="'.aurl('colors/'.$color->id.'/edit').'" class="btn btn-info edit"><i class="fa fa-edit"></i></a>';
			})
			->addColumn('delete', function($color){
                $html = '<form style="margin-bottom: 0"  method="post" action="' . route('colors.destroy', $color->id) . '">';
			...
			->addColumn('color', function($color){
				return '<span style="width: 25px;height: 25px;background-color: '.$color->color.';display:block"></span>';
			})
