...
class WeightsDatatable extends DataTable {
	...
	public function dataTable($query) {
		return datatables($query)
			->addColumn('checkbox', function($weight){
				return '<input type="checkbox" name="item[]" value="'.$weight->id.'" class="item_checkbox"/>';
			})	
			->addColumn('edit', function($weight){
				return '<a href="'.aurl('weights/'.$weight->id.'/edit').'" class="btn btn-info edit"><i class="fa fa-edit"></i></a>';
			})
			->addColumn('delete', function($weight){
                $html = '<form style="margin-bottom: 0"  method="post" action="' . route('weights.destroy', $weight->id) . '">';
